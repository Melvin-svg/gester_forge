...
class CyberHandTracker {
  ...
  tickFrame() {
    ...
    // HAVE_CURRENT_DATA (2) is enough to grab a frame; waiting for
    // HAVE_ENOUGH_DATA (4) can stall indefinitely on a throttled stream.
    if (this.video.readyState >= 2) {
      ...
      // Retry next frame when video data becomes available
      requestAnimationFrame(() => this.tickFrame());
    }
  }
  ...
  processResults(results) {
    ...
    if (handIndex >= 0) {
      ...
      // If hand is lost, reset filters to prevent lag spikes when hand returns
      this.resetFilters();
    }

    // Pass data forward to the main application callback
    this.onResults({
...
window.CyberHandTracker = CyberHandTracker;
